# SuperSpec - Superpowers + Spec-Kit 整合開發流程(修正版 v2026.04.21a)
...
> **重要**:Superpowers 的【SP指令】 /superpowers:<brainstorm/write-plan/execute-plan> 觸發方式,已全面棄用
...
| 步驟 | 標籤 | 動作 | 說明 |
|---|---|---|---|
| 0.1 | 【Shell端】 | `uvx --from git+https://github.com/github/spec-kit.git@<tag> specify init <project> --integration claude` | 專案裝上 **Spec-Kit** |
| 0.2 | 【Shell端】 | `cd <project> && git config user.name '<name>' && git config user.email '<email>' && echo '.claude/' > .gitignore` | 專案初始化 **Git** |
| 0.2 | 【CC原生】| `/plugin marketplace add obra/superpowers-marketplace` (全域安裝) | **僅需執行一次** |
| 0.3 | 【CC原生】| `/plugin install superpowers@superpowers-marketplace` (全域安裝) | **僅需執行一次** |

> 🔵 **建議**:`specify init` 請釘版本 tag(例如 `@v0.x.x`),不然會拉 `main`,行為可能隨上游變動。官方 README 推薦這麼做。
>
> 跑完 `specify init` 後,`/speckit.*` 系列指令會被寫進 `.claude/commands/`,Claude Code 重啟後就能用。
...
| 步驟 | 標籤 | 動作 | 說明 |
|---|---|---|---|
| 1.1 | 【SP指令】 | `superpowers:brainstorming <想做什麼>` | 明確觸發【SP技能】`brainstorming` |
...
>
> 🔵 **關於 1.4 的 `/clear`**:忍者工坊原文只說「碰到 rate limit 就等,下個 session 接著做」,沒強制「每階段必 clear」。此處的 `/clear` 是本文主動的積極策略,目的是讓下個階段起始 context 乾淨——若你一路順暢沒撞 rate limit,跳過也可以。
...
| 步驟 | 標籤 | 動作 | 說明 |
|---|---|---|---|
| 2.0 | 【SK指令】 | `/speckit.constitution` →【SK檔案】`.specify/memory/constitution.md` | 設定專案治理原則 |
...
| 2.2 | 【SK指令】 | `/speckit.clarify` (官方列為 optional 🔵) | 結構化澄清模糊處,記錄到 Clarifications 段 |
...
└── memory/
    └── constitution.md
specs/
└── <NNN-feature-name>/       ← 編號前綴由 spec-kit 自動加(例:001-photo-albums)
    ├── spec.md       ← 階段 3 spec compliance 審查的真相來源
    ├── plan.md
    └── tasks.md      ← 階段 3 execute-plan 的執行輸入
```
...
| 步驟 | 標籤 | 動作 | 說明 |
|---|---|---|---|
| 3.0 | 【SP技能】 | `superpowers:verification-before-completion` 🔵 | 確認測試 baseline 全綠 |
...
| 步驟 | 標籤 | 動作 | 說明 |
|---|---|---|---|
| 3.1 | 【SP指令】 | `superpowers:executing-plans` | 把 spec-kit 的 `tasks.md` 指給 superpowers,啟動執行 |
...
| 步驟 | 標籤 | 動作 | 說明 |
|---|---|---|---|
| 3.2.1 | 【SP技能】 | `subagent-driven-development` superpowers 協調器 | 派遣**全新 implementer subagent**(乾淨 context window) |
| 3.2.2 | 【SP技能】 | `test-driven-development` implementer subagent | 寫 code,嚴格走 Red→Green→Refactor |
| 3.2.3-a | 【SP技能】 | `superpowers:code-reviewer` Spec compliance reviewer | **拿著 `spec.md` 對照實作**——做出來的東西符合規格嗎? |
| 3.2.3-b | 【SP技能】 | `superpowers:code-reviewer` Code quality reviewer | code 寫得好不好?測試夠不夠?(這階段不看 spec) |
| 3.2.4 | 【SP技能】 | `subagent-driven-development` superpowers 協調器 | 收到 subagent 回報,任務完成,進下一個任務 |
...
>
> **!!!注意!!!**:此階段結束,尚不要進行分支`worktree`結束的建議(superpowers:executing-plans結束時它會提示關閉`worktree`的選項)
...
# === 階段 0:一次性設定 ===
# Shell端(釘 tag 建議):
$ uvx --from git+https://github.com/github/spec-kit.git@v0.x.x specify init my-app --integration claude
...
你:「superpowers:brainstorming 我想在 footer 加深色模式切換」   # 【SP指令】(三個真指令之一,對應 brainstorming skill)
    └─ 進入蘇格拉底式問答
你: (回答 N 輪問題,確認設計草稿)
...
你:「/speckit.plan 用 Tailwind + CSS variables 實作」  # 【SK指令】
...
你:「superpowers:executing-plans 讀 specs/<NNN-feature-name>/tasks.md,
    ...
    └─【SP 指令】 啟動整個 loop
       ...
       ├─ 2. implementer 用 test-driven-developmentd 走 Red→Green→Refactor   # 【SP技能】
       ├─ 3-a. spec compliance reviewer 拿 spec.md 對照實作           # 【SK檔案】真相來源
       ├─ 3-b. code quality reviewer 看 code 品質
       └─ 4. 任務完成,進下一個

# === 階段 4:收尾 🔵 ===
# 以下三步都是 skill,用自然語言觸發(沒有同名斜線指令):
你:「requesting-code-review 對照 specs/<NNN-feature-name>/spec.md 做 review」
    └─【SP技能】 requesting-code-review
你:「superpowers:requesting-code-review 幫我做完成前驗證,跑全測試、lint、型別檢查」
    └─【SP技能】 verification-before-completion
你:「superpowers:finishing-a-development-branch 收尾,開 PR、清理 worktree」
    └─【SP技能】 finishing-a-development-branch
```
